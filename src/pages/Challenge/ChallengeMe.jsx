...
export default class ChallengeMe extends Component {
  constructor() {
    super();
    const score = 0
    this.state = {
    ...
    return (
      <div>
        ...
        <JumbotronSmall name={'Challenge'}/>
          <h3>You begin, pick a number between 1 and 10, whoever gets to 100 first, wins.</h3>
          <br></br>
          <h3>The current sum is: {this.state.score}</h3>
          {
            this.endGame(this.state.score) &&
            <h1>{this.state.history.length % 2 === 0 ? 'You lost' : 'You won!'}</h1>
          }
          <br></br>
          {
            !this.endGame(this.state.score) &&
        ...
        }
        {
            this.state.history.map((log, index) =>
              <h4 key={index}>{index % 2 === 0 ? 'You' : 'PC' } choose {log}. </h4>
            )
          }
          <br></br>
          <br></br>
